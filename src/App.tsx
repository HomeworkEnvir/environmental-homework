    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          // Сортируем по годам, чтобы график шел хронологически
          ...
          const formatted = sortedData.map(item => ({
            ...
            value: `${(parseFloat(item.OBS_VALUE.toString()) / 1000000).toFixed(1)}M`, // Пример конвертации в миллионы, если в базе целые числа
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-cyan-50 to-teal-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 text-slate-900 dark:text-slate-100 font-sans selection:bg-blue-200 transition-colors duration-500">
      ...
      <section className="max-w-6xl mx-auto px-8 py-16 animate-fade-in">
        <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-8 md:p-12 shadow-2xl border border-slate-100 dark:border-slate-700">
          <h2 className="text-3xl font-black mb-8 flex items-center dark:text-white">
            <TrendingUp className="w-8 h-8 mr-3 text-blue-600" /> Global Plastic Leakage Timeline
          </h2>
          ...
          <div className="relative pt-10">
            {isLoading ? (
              <div className="h-64 flex flex-col items-center justify-center text-slate-400">
                ...
                <p className="uppercase tracking-widest text-xs font-bold">Synchronizing with OECD Database...</p>
              </div>
            ) : error ? (
              <div className="h-64 flex items-center justify-center text-red-500 font-mono">{error}</div>
            ) : (
              <div className="overflow-x-auto custom-scrollbar pb-6">
                {/* Динамическая ширина в зависимости от количества элементов */}
                <div 
                  className="flex items-end justify-between gap-2 min-w-full h-72 border-b-2 border-slate-100 dark:border-slate-700 px-4"
                  style={{ width: `${realGrowthData.length * 40}px` }} 
                >
                  {realGrowthData.map((d, i) => (
                    <div key={i} className="flex flex-col items-center group relative" style={{ width: '32px' }}>
                      ...
                      {/* Столбец */}
                      <div 
                        className="w-full bg-gradient-to-t from-blue-600 to-cyan-400 rounded-t-md shadow-md transition-all duration-500 group-hover:brightness-125 group-hover:scale-x-110"
                        style={{ height: `${d.percentHeight}%`, minHeight: '4px' }}
                      />
                      
                      {/* Подпись года */}
                      <span className="text-[10px] font-bold text-slate-400 mt-4 transform -rotate-45 origin-top-left lg:rotate-0 lg:origin-center">
                        {d.year}
                      </span>
                    </div>
                  ))}
      ...
      <section className="max-w-6xl mx-auto px-8 py-16">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="text-4xl font-black mb-6 dark:text-white">Systemic Impact</h2>
