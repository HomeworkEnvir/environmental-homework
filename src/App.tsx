      ...
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          const getPercentHeight = (current: number) => {
            if (diff === 0) return 80;
            // МИНИМУМ 20%, чтобы столбики не исчезали, МАКСИМУМ 100%
                ...
                <div className="flex items-end justify-between gap-4 min-w-full h-80 border-b-2 border-slate-100 dark:border-slate-700 px-6">
                  {realGrowthData.map((d, i) => (
                    <div key={i} className="flex-1 flex flex-col items-center group relative h-full justify-end max-w-[100px]">
                      ...
                      <div 
                        className={`w-full rounded-t-xl shadow-lg transition-all duration-1000 ease-out animate-grow hover:brightness-110 ${
                          d.isLastYear 
