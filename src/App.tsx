    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          // 3. Форматирование для визуализации
          ...
          const formatted = years.map(year => {
            ...
            return {
              year: `${year}s`,
              // Округляем до целого числа и добавляем M
              value: `${Math.round(val)}M`,
              // Расчет высоты с минимальным порогом 4% для видимости
              percentHeight: maxValue > 0 ? Math.max((val / maxValue) * 100, 4) : 4
            };
          });
        ...
        <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-8 md:p-12 shadow-2xl border border-slate-100 dark:border-slate-700">
          <div className="mb-10">
            <h2 className="text-3xl font-black mb-4 flex items-center dark:text-white">
              ...
              5-Year Leakage Trends
            </h2>
            <p className="text-slate-600 dark:text-slate-300 max-w-3xl leading-relaxed">
              This chart visualizes global plastic leakage grouped by five-year periods. Data reflects total millions of tons failing to be contained by waste management systems.
            </p>
          </div>
          ...
          <div className="relative">
            {isLoading ? (
              <div className="h-72 flex flex-col items-center justify-center text-slate-400">
                ...
                <p className="text-xs font-bold uppercase tracking-widest">Processing OECD Periods...</p>
              </div>
            ) : error ? (
              ...
              <div className="overflow-x-auto custom-scrollbar pb-8">
                <div 
                  className="flex items-end justify-around gap-4 min-w-full h-72 border-b-2 border-slate-100 dark:border-slate-700 px-4"
                >
                  {realGrowthData.map((d, i) => (
                    <div key={i} className="flex flex-col items-center group relative flex-1 max-w-[80px]">
                      ...
                      {/* Столбик */}
                      <div 
                        className="w-full bg-gradient-to-t from-blue-700 to-cyan-400 rounded-t-md shadow-lg transition-all duration-700 ease-out group-hover:brightness-110"
