    ...
    const fetchData = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          // Находим максимум для пропорционального отображения графиков
          const values = data.map(d => parseFloat(d.OBS_VALUE.toString()));
          const maxValue = Math.max(...values);
          
          const formatted = data.map(item => ({
            year: item.TIME_PERIOD,
            value: `${Math.round(parseFloat(item.OBS_VALUE.toString()))}M`,
            // Рассчитываем высоту в % относительно максимума
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 items-center bg-white dark:bg-slate-800 rounded-[2.5rem] p-8 md:p-12 shadow-2xl border border-slate-100 dark:border-slate-700">
          <div className="lg:col-span-2">
            ...
            <div className="space-y-4 text-slate-600 dark:text-slate-300 leading-relaxed">
              <p>
                This chart visualizes <strong>global plastic leakage</strong> from 1990 to 2019. Each bar represents millions of tons of plastic failing to be contained by waste management systems.
              </p>
              <p>
                The data is pulled from a live Python backend that processes official OECD XML datasets in real-time.
              </p>
            </div>
          </div>
          ...
          <div className="lg:col-span-3">
            <div className="flex justify-between items-end mb-8">
              <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Leakage (Million Tons)</span>
            </div>
            ...
            <div className="flex items-end justify-between gap-1 h-64 border-b-2 border-slate-100 dark:border-slate-700 pb-2 overflow-x-auto scrollbar-hide">
              {isLoading ? (
                <div className="w-full flex flex-col items-center justify-center h-full text-slate-400">
                  <Loader2 className="w-8 h-8 animate-spin mb-2" />
                  <p className="text-[10px] font-bold uppercase tracking-widest">Fetching from Render...</p>
                </div>
              ) : error ? (
                <div className="w-full text-center text-red-500 text-xs font-mono">{error}</div>
              ) : (
                realGrowthData.map((d, i) => (
                  <div key={i} className="flex flex-col items-center flex-1 group min-w-[14px]">
                    <div 
                      className="w-full max-w-[24px] bg-gradient-to-t from-blue-700 to-cyan-400 rounded-t-sm shadow-lg transition-all duration-500 group-hover:brightness-125 relative"
                      style={{ height: `${d.percentHeight}%` }}
                    >
                      <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[10px] font-bold py-1 px-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50">
                        {d.value}
                      </div>
                    </div>
                    <span className="text-[9px] font-bold text-slate-400 mt-4 rotate-45 lg:rotate-0">
